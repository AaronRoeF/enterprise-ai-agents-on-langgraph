...
- **Editorial discipline** — tightened citation discipline across the body between v1.0.0 and v1.1.0: ASCII → Unicode arrow whitelist transition with column-preserving arrow conventions for diagram legibility; first-use acronym expansion discipline (24 expansions); OSI-aligned 10-tier stack numbering (Tier 1 = Compute, Tier 10 = LLM per ISO/IEC 7498-1); cross-tier glossary consistency. A pre-publication reader review by senior practitioner reviewers (Sales Engineer / Solution Consultant / Product Manager / Forward Deployed Engineer / Enterprise Architect / CISO / CTO lenses) strengthened §2.3 recipes (per-recipe Audit-Evidence Patterns + failure-mode runbooks + 30/60/90 deployment postures); §2.4 Identity (SPIFFE/SPIRE expansion with SVID-type disambiguation and the SPIRE → RFC 8693 → DPoP worked composition; worked agent-JWT claims set + verifier-side validation order; MCP server adoption matrix); §2.7.2 Cat-1 cross-tenant depth (per-store wire-level configuration snippets for pgvector / Pinecone / Weaviate / Qdrant / Elasticsearch / Snowflake Cortex / Databricks Vector Search); §2.7.2 Cat-6 supply-chain attestation (SLSA / Sigstore / cosign / in-toto / CycloneDX / SPDX); §2.8.3 deployment-shape decision matrix (rows naming customer KMS, observability pipeline, identity broker, kernel/eBPF/hypervisor dependency); §2.8.5 exit-portability skeleton for DORA Art. 28 exit-plan artifacts; §2.9.7.2 WAF-pillar / NIST 800-207 cross-walk; broadened §2.4.11 Doctolib error-states box with audit-emission requirements + verifier-side validation order.
...
90-day update cycle committed. The next minor release (**v1.2.0**, expected late August 2026) lands the lower-priority items deferred from v1.1.0: NIST AI RMF row in §2.5; FFIEC IT Booklets named individually; OAuth procurement-narrative layering callout; five sovereignty axes lifted earlier in the book; §2.7.4 CVE column + customer-language version column + DPD-vs-MyCity distinguishing footnote; "evidence-zero, structural-fit-only" glossary promotion + PRD-use guidance; §2.10 quiz PM-track + SC-specific blocks; named-framework exclusion-appendix callouts; §2.3.6 Elastic `[reference design]` co-anchor; §2.9.7 Rosetta Stone pull-out card; memory eval-harness primitives in §2.3.0.3; Okta shadow-agent discovery protocol.

Quarterly glossary audit + named-primitive change tracking are part of the documented revision cycle. Community contributions welcome — see [`CONTRIBUTING.md`](CONTRIBUTING.md).
